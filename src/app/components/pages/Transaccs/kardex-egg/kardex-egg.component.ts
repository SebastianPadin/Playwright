...
export class KardexEggComponent implements OnInit {
    ...
    this.typeKardexService.listAll().subscribe(
      (data) => {
        ...
        if (this.kardexList.length > 0) {
          this.selectedKardex = this.kardexList[0].id;
          this.selectedKardexData = this.kardexList[0];
          this.filterMovements();  // Filtrar los movimientos desde el inicio
        }
      },
  ...
  loadMovements(): void {
    this.movementKardexService.getAll().subscribe(
      (data) => {
        ...
        this.filterMovements();  // Filtrar los movimientos después de cargarlos
      },
      (error) => console.error('Error al obtener los movimientos del Kardex', error)
    );
  }
        ...
        if (result.isConfirmed) {
            this.movementKardexService.delete(movementId).subscribe({
                next: () => {
                    ...
                    this.loadMovements(); // Recarga la lista después de eliminar
                },
                error: (err) => {
  ...
  showModalForCreation(): void {
    this.editMode = false;
    this.currentMovementKardex = {} as MovementKardex;
